...
const ForeCast = (props) => {
  ...
  const [foreCastHum, SetforeCastHum] = useState(false);
        ...
        return new Promise(async function (resolve, reject) {
          ...
          if (senObj.message != "Something went wrong") {
            for (let y = 0; y < 8; y++) {
              ...
              let timealone = timestr.substr(t + 1, timestr.length - 3);
    ...
    <View style={styles.ForeContainer}>
      <View style={styles.ForeHeading}>
        <Animatable.View style={styles.ForeIcon}>
          <MaterialCommunityIcons name="water" color={"#FFF"} size={37} />
        </Animatable.View>
        <Animatable.View style={styles.ForeText}>
          <Text style={styles.ForeTextLabel}>Humidity Forecast</Text>
        </Animatable.View>
      </View>
